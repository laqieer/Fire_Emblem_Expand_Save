...
// only verify from the start of the sector, skip verify for not sector aligned data
u32 VerifyFlash_Wrapper(u8 *src,u8 *tgt,u32 size)
{
	if((u32)tgt % flash->sector.size == 0)
		return VerifyFlash((u32)tgt / flash->sector.size, src, size);
	return 0;
}
