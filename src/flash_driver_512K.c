#include "flash_driver_512K.h"

__attribute__((section(".libraryIdentifier")))
const char libraryIdentifier[] = "FLASH_V131";
...
void WriteFlash(u8 *src, u8 *dst, u32 size)
{
    ...
    gba_memcpy(&sector_buffer[addr_Flash - FLASH_ADR - SECTOR_SIZE * sector_number_start], src, byte_number);
    ...
    if(sector_number_end > sector_number_start)
    {
        ...
        gba_memcpy(sector_buffer, src + byte_number + SECTOR_SIZE * (sector_number_end - sector_number_start - 1), size - byte_number - SECTOR_SIZE * (sector_number_end - sector_number_start - 1));
...
{
	if((u32)tgt % SECTOR_SIZE == 0)
		return VerifyFlash((u32)tgt / SECTOR_SIZE, src, size);
	return 0;
}
